...
namespace DomainModel
{
    ...
    public class ftp
    {
        ...
        public static void UploadFtpFile(string ip, string user, string pass, string folderName, string fileName)
        {
            try
            {
                string str = Path.GetFileName(fileName);
                FtpWebRequest request = WebRequest.Create(new Uri(string.Format("ftp://{0}/{1}/{2}", ip, folderName, str))) as FtpWebRequest;
        ...
        public static void UploadFtpFile_No_Folder(string ip, string user, string pass, string fileName)
        {
            try
            {
                string str = Path.GetFileName(fileName);
                FtpWebRequest request = WebRequest.Create(new Uri(string.Format("ftp://{0}/{1}", ip, str))) as FtpWebRequest;
                request.Method = WebRequestMethods.Ftp.UploadFile;
                request.UseBinary = true;
