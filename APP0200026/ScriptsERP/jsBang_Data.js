...
function Bang_KhoiTao() {
    ...
    Bang_arrClassCot = Bang_LayMang1ChieuGiaTri('idXauClassCot');
...
function Bang_GanGiaTriThatChoO(h, c, GiaTri) {
    if (h > -1 && c > -1) {
        ...
        if (Bang_arrGiaTri[h][c] != GiaTri) {
            ...
            var ThayDoiCu = Bang_arrThayDoi[h][c];
            Bang_arrThayDoi[h][c] = true;
            Bang_arrGiaTri[h][c] = GiaTri;
            Bang_HienThiDuLieuO(h, c);

            if (Bang_arrMaCot[c] == "sMauSac" || Bang_arrMaCot[c] == "sFontColor" || Bang_arrMaCot[c] == "sFontBold" || !ThayDoiCu) {
                Bang_HienThiDuLieu();
            }
...
function Bang_LayDuLieuHienThiCuaO(h, c) {
    ...
    switch (Bang_arrType[c]) {
        ...
        case 2:
            ...
            if (checkbox_value == "1") {
                GTHienThi = '<span title="Đạt" style="width:' + Bang_arrDoRongCot[c] + 'px;" class="check">&nbsp;</span>';
            }
            else {
                GTHienThi = '<span title="Không đạt" style="width:' + Bang_arrDoRongCot[c] + 'px;">&nbsp;</span>';
            }
            break;
        ...
        default:
            ...
            Bang_arrHienThi[h][c] = Bang_DinhDangHienThiO(h, c);
            GTHienThi = '<span title="' + Bang_arrGiaTri[h][c] + '" style="width:' + Bang_arrDoRongCot[c] + 'px;">' + Bang_arrHienThi[h][c] + '</span>';
            break;
    }
    return GTHienThi;
}
...
function Bang_HienThiDuLieu() {
    ...
    var cFocus = -1;
    if (Bang_keys != null) {
        cFocus = Bang_keys.Col();
        hFocus = Bang_keys.Row();
    }
    ...
    for (var h = hMin; h < hMax; h++) {
        ...
        for (var c = 0; c < Bang_nC; c++) {
            var cell_class = Bang_arrClassCot[c];
            var j = AnhXaCot_DuLieu_Fixed(c);
            if (j >= 0) {
                if (cell_class != '') {
                    $(Bang_arrCell_Fixed[n][j]).addClass(cell_class);
                }
            }
            j = AnhXaCot_DuLieu_Slide(c);
            if (j >= 0 && !(cFocus == c && hFocus == h)) {
                if (cell_class != '') {
                    $(Bang_arrCell_Slide[n][j]).addClass(cell_class);
                }
            }
            //thay doi mau hong sau khi da thay doi du lieu
            //if (j >= 0) {
            //    if (Bang_arrThayDoi[h][c]) {
            //        $(Bang_arrCell_Slide[n][j]).css('background-color', '#f8d2f8');
            //    }
            //}
